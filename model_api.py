...
def NN_regression(input_cols, output_cols, hidden_neurons, hidden_layers, lr=3e-3, gamma=1e-5, verbose=False):
	'''
		input_cols: number of input columns
		output_cols: number of predicted variables
	...
	'''
	X = keras.layers.Input(shape=(input_cols))
	...
	classifier = keras.layers.Dense(output_cols, activation="linear", kernel_regularizer=keras.regularizers.l2(gamma))(hidden)
...
# Retrieve Data Set
def get_standard_data(): # Import the standard kaggle dataset!
	...
	# Return normalization factors to apply to other datasets!
